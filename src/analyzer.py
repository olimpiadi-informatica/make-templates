...
if argv[3] not in dir(targets):
    print("[ERROR] Target '%s' not supported" % argv[3])
    exit(1)

error_listener = FailOnError()
input_stream = InputStream(''.join(open(argv[1]).readlines()))
try:
...
if len(err) == 0:
    spec = importlib.util.spec_from_file_location('limits', argv[2])
    limits = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(limits)
    print(getattr(targets, argv[3]).generate(yaml.safe_load(''.join(open('task.yaml', 'r').readlines()))['name'], res, argv[4], limits.__dict__))
